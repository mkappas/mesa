...
PREPROCESS := ifort -cpp -E
FCOMPILE := ifort $(_FFLAGS) -c
FCOMPILE_MODULE := ifort $(_FFLAGS) -w -c -fsyntax-only
FCOMPILE_FIXED := ifort $(FFLAGS_FIXED) -c
FCOMPILE_MODULE_FIXED:= ifort $(FFLAGS_FIXED) -w -c -fsyntax-only
...
LIB_TOOL_DYNAMIC := ifort -shared $(FLAGS_OPENMP) $(FLAGS_COVERAGE)
EXECUTABLE := ifort $(FLAGS_OPENMP) $(FLAGS_COVERAGE)
